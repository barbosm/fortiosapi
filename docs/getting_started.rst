...
This document will show you how to get up and running with the FortiOS API
client. You will have obtained a FortiGate version and Serial Number using 
python in 5 minutes.
...
* Access to a FortiGate unit;
* Python 2.4 or 3.6 installed


Installation
---------------

If your computer has internet access you can use the Python Package manager
to install fortiosapi

.. code-block:: bash

  $ sudo pip install fortiosapi

.. Note:: You will likely notice pip install some dependencies.


Enable API access
------------------

Make sure your FortiGate unit has HTTPS access enabled:

.. code-block:: bash

  $ sudo pip install fortiosapi

You should be able to test it by acessing the URL:

  http://<YOUR_DEVICE_IP>/api/v2/cmdb/firewall/policy

.. Note:: This should display your firewall policy in a JSON formated response
